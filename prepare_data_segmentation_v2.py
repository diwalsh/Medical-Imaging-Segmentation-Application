    ...
    if dimension != '2.5d':
        ...
        for folder in CASE_FOLDERS:
            ...
            if folder[4:] in test_patients:
                create_and_write_img_msk(files, ids, ROOT_TEST_IMG_DIR, ROOT_TEST_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Test :: {folder}")
            else:
                train_files, valid_files, train_ids, valid_ids = train_test_split(files, ids, train_size=0.85, random_state=42, shuffle=True)
                create_and_write_img_msk(train_files, train_ids, ROOT_TRAIN_IMG_DIR, ROOT_TRAIN_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Train :: {folder}")
                create_and_write_img_msk(valid_files, valid_ids, ROOT_VALID_IMG_DIR, ROOT_VALID_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Valid :: {folder}")
    else:
        for folder in CASE_FOLDERS:
            files, ids = get_folder_files_2p5d(folder_path=os.path.join(ORIG_IMG_DIR, folder), only_IDS=oIDS, stride=stride)
            if folder[4:] in test_patients:
                create_and_write_img_msk_2p5d(files, ids, ROOT_TEST_IMG_DIR, ROOT_TEST_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Test :: {folder}")
            else:
                train_files, valid_files, train_ids, valid_ids = train_test_split(files, ids, train_size=0.85, random_state=42, shuffle=True)
                create_and_write_img_msk(train_files, train_ids, ROOT_TRAIN_IMG_DIR, ROOT_TRAIN_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Train :: {folder}")
                create_and_write_img_msk(valid_files, valid_ids, ROOT_VALID_IMG_DIR, ROOT_VALID_MSK_DIR, main_df=oDF, mask_rgb=mask_rgb, desc=f"Valid :: {folder}")
