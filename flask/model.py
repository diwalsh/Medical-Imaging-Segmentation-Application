...
id2color = {
    ...
    1: (255, 140, 140),  # Stomach
    2: (255, 102, 102),  # Small Bowel
    3: (255, 51, 51),  # large Bowel
}
...
def inference(model, class_model, image_paths, img_size, batch_size=24, device="cpu"):
    ...
    for batch_idx in range(num_batches):
        ...
        for idx in range(start_idx, end_idx):
            ...
            batch_images.append(image_norm)
        ...
        if len(true_idxs) > 0:
            predictions = model(batch_images[true_idxs])
        ...
        for i in range(len(batch_images)):
            batch_img_np = batch_images[i].permute(1, 2, 0).numpy()  # Convert to numpy
